...
if (isset ($_GET['delete'])){
    $id = $_GET['delete'];
    $mysqli->query("DELETE FROM REPORTS WHERE report_id=$id");
}
if (!isset($_GET['page'])) {
    $page = 1;
} else {
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <thead >
                                            ...
                                            <tbody>
                                                ...
                                                from (REPORTS JOIN CLIENT C ON C.client_id=reporter_id) where report_is_seen=1 "
                                                );
                                                $statment->execute();
                                                ...
                                                <tr>
                                                    ...
                                                    <td width=20%>
                                                    <section ><details ><summary ><b>REREAD</b></summary><?php echo $row['report_content']; ?></details></section>
                                                    </td>
                                                    <td><a href="sreport.php?delete=<?php echo $row['report_id']; ?>">DELETE</a></td>
                                                </tr>
                                                <?php } ?>
                                            </tbody>
                                        </table>
                                        ...
                                        <nav aria-label="...">
                                            ...
                                            for ($p=1;$p<=$number_of_pages;$p++) {
                                                if ($p==$page) {?>
                                                    ...
                                                    echo '<a class="page-link" href="TEST S REPORTS.php?page=' . $p . '" >' .  $p . '</a> ';}
                                                    else { ?> 
